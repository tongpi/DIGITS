...
class ImageClassificationDatasetForm(ImageDatasetForm):
    ...
    backend = wtforms.SelectField(lgt('DB backend'),
                                  choices=[
                                      ('lmdb', 'LMDB'),
                                      ('hdf5', 'HDF5')
                                  ],
    ...
    compression = utils.forms.SelectField(
        ...
        default='none',
        tooltip=lgt('Compressing the dataset may significantly decrease the size '
    ...
    method = wtforms.SelectField(lgt(u'Dataset type'),
                                 choices=[
                                     ('folder', lgt('Folder')),
                                     ('textfile', lgt('Textfiles')),
                                 ],
                                 default='folder',
                                 )
        ...
        if not field.data:
            ...
            try:
                ...
                if r.status_code not in [requests.codes.ok, requests.codes.moved, requests.codes.found]:
                    raise validators.ValidationError(lgt('URL not found'))
            except Exception as e:
                raise validators.ValidationError(lgt('Caught %(name)s while checking URL: %(e)s', name=type(e).__name__, e=e))
            else:
                return True
        else:
            ...
            if not os.path.exists(field.data) or not os.path.isdir(field.data):
                raise validators.ValidationError(lgt('Folder does not exist'))
            elif not os.path.isabs(field.data):
                raise validators.ValidationError(lgt('Filesystem path is not absolute'))
            else:
                return True
    ...
    folder_train = utils.forms.StringField(
        lgt(u'Training Images'),
        validators=[
            validate_required_iff(method='folder'),
            validate_folder_path,
        ],
        tooltip=lgt('Indicate a folder which holds subfolders full of images. '
    ...
    folder_train_min_per_class = utils.forms.IntegerField(
        lgt(u'Minimum samples per class'),
        default=2,
        validators=[
            validators.Optional(),
            validators.NumberRange(min=1),
        ],
        tooltip=lgt('You can choose to specify a minimum number of samples per class. '
    ...
    has_test_folder = wtforms.BooleanField(
        lgt('Separate test images folder'),
        default=False,
        validators=[
            validate_required_iff(method='folder')
        ]
    ...
    textfile_train_folder = wtforms.StringField(lgt(u'Training images folder'))
    ...
    textfile_use_val = wtforms.BooleanField(lgt(u'Validation set'),
                                            default=True,
                                            validators=[
                                                validate_required_iff(method='textfile')
                                            ]
                                            )
    textfile_val_images = utils.forms.FileField(lgt(u'Validation images'),
                                                validators=[
                                                    validate_required_iff(
                                                        method='textfile',
    ...
    textfile_local_val_images = wtforms.StringField(lgt(u'Validation images'),
                                                    validators=[
                                                        validate_required_iff(
                                                            method='textfile',
                                                            textfile_use_val=True,
    ...
    textfile_val_folder = wtforms.StringField(lgt(u'Validation images folder'))
    ...
    textfile_use_test = wtforms.BooleanField(lgt(u'Test set'),
                                             default=False,
                                             validators=[
                                                 validate_required_iff(method='textfile')
                                             ]
                                             )
    textfile_test_images = utils.forms.FileField(lgt(u'Test images'),
                                                 validators=[
                                                     validate_required_iff(
                                                         method='textfile',
    ...
    textfile_local_test_images = wtforms.StringField(lgt(u'Test images'),
                                                     validators=[
                                                         validate_required_iff(
                                                             method='textfile',
                                                             textfile_use_test=True,
    ...
    textfile_test_folder = wtforms.StringField(lgt(u'Test images folder'))
    ...
    textfile_shuffle = utils.forms.SelectField(
        ...
        coerce=int,
        default=1,
        tooltip=lgt("Shuffle the list[s] of images before creating the database.")
    )
