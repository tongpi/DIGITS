...
class ParseFolderTask(Task):
    ...
    def __init__(self, folder, **kwargs):
        ...
        if percent_val is None:
            ...
            if pct < 0:
                ...
                raise ValueError(lgt('percent_val must not exceed 100'))
        ...
        if percent_test is None:
            ...
            if pct < 0:
                ...
                raise ValueError(lgt('percent_test must not exceed 100'))
            self.percent_test = pct

        if percent_val is not None and percent_test is not None and percent_val + percent_test > 100:
            raise ValueError(lgt('the sum of percent_val and percent_test must not exceed 100'))
    ...
    def name(self):
        ...
        return lgt('Parse Folder (%(name)s)', name=('/'.join(sets)))
