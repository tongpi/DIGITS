...
class DatasetForm(Form):
    ...
    def validate_file_path(form, field):
        if not field.data:
            ...
            if not os.path.exists(field.data) and not os.path.isdir(field.data):
                raise validators.ValidationError(
                    lgt('File does not exist or is not reachable'))
            else:
                return True
    ...
    colormap_method = utils.forms.SelectField(
        lgt('Color map specification'),
        choices=[
            ('label', lgt('From label image')),
            ('textfile', lgt('From text file')),
        ],
        default='label',
        tooltip=lgt("Specify how to map class IDs to colors. Select 'From label "
    ...
    colormap_text_file = utils.forms.StringField(
        ...
        tooltip=lgt("Specify color/class mappings through a text file. "
