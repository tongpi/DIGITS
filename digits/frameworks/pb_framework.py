...
from digits.model.tasks import PBTrainTask
from digits.utils import subclass, override, constants


@subclass
class PBFramework(Framework):
    """
    ...
    def __init__(self):
        super(PBFramework, self).__init__()
        # id must be unique
    ...
    def create_train_task(self, **kwargs):
        ...
        create train task
        """
        return PBTrainTask(framework_id=self.framework_id, **kwargs)
        ...
        try:  # do this in a try..finally clause to make sure we delete the temp file
            ...
            while p.poll() is None:
                for line in utils.nonblocking_readlines(p.stdout):
                    timestamp, level, message = PBTrainTask.preprocess_output_tensorflow(line.strip())
                    if line is not None:
                        stdout_log += line
            if p.returncode:
