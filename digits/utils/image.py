...
def load_image(path):
    ...
    if is_url(path):
        ...
        try:
            ...
            raise errors.LoadImageError, lgt('IOError: Trying to load "%(path)s": %(message)s', path=path,
                                             message=e.message)
    else:
        raise errors.LoadImageError, lgt('"%(path)s" not found', path=path)
    ...
    if image.mode in ['L', 'RGB']:
        ...
        raise errors.LoadImageError, lgt('Image mode "%(mode)s" not supported', mode=image.mode)
...
def upscale(image, ratio):
    ...
    if ratio < 1:
        raise ValueError(lgt('Ratio must be greater than 1 (ratio=%(ratio)f)', ratio=ratio))
...
def image_to_array(image,
    ...
    if channels not in [None, 1, 3, 4]:
        raise ValueError(lgt('unsupported number of channels: %(channels)s', channels=channels))
    ...
    if isinstance(image, PIL.Image.Image):
        ...
        if channels is None:
            ...
            if image_mode not in ['L', 'RGB', 'RGBA']:
                raise ValueError(lgt('unknown image mode "%(image_mode)s"', image_mode=image_mode))
        elif channels == 1:
        ...
        if channels is None:
            if not (image.ndim == 2 or (image.ndim == 3 and image.shape[2] in [3, 4])):
                raise ValueError(lgt('invalid image shape: %(image_shape)s', image_shape=image.shape))
        elif channels == 1:
            if image.ndim != 2:
        ...
        raise ValueError(lgt('resize_image() expected a PIL.Image.Image or a numpy.ndarray'))
...
def resize_image(image, height, width,
    ...
    if resize_mode not in ['crop', 'squash', 'fill', 'half_crop']:
        raise ValueError(lgt('resize_mode "%(resize_mode)s" not supported', resize_mode=resize_mode))
    ...
    if resize_mode == 'squash' or width_ratio == height_ratio:
        ...
        if resize_mode == 'fill':
            ...
            raise Exception(lgt('unrecognized resize_mode "%(resize_mode)s"', resize_mode=resize_mode))
...
def embed_image_html(image):
    ...
    if image is None:
        ...
        raise ValueError(lgt('image must be a PIL.Image or a np.ndarray'))
...
def get_layer_vis_square(data,
    ...
    if channel_order not in ['RGB', 'BGR']:
        raise ValueError(lgt('Unsupported channel_order %(channel_order)s', channel_order=channel_order))
    if data.ndim == 1:
        ...
        raise RuntimeError(lgt('unrecognized data shape: %(data_shape)s', data_shap=data.shape))
